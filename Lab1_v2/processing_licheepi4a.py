...
#Licheepi4a
print("\nResults on Licheepi4a ", file=open("result/licheepi4a_result.txt", "w"))
plt.clf()
gcc_x = [[],[],[],[]]
gcc_y = [[],[],[],[]]
for i in range(4):
    with open("data/LicheePi4A/gcc-O" + str(i) + ".csv", "r") as f:
        j = 1
        for ch in f:
            gcc_x[i].append(j*10)
...
print("licheepi_gcc_O0 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(gcc_x[1], gcc_y[1], 1)
p = np.poly1d (z)
plt.plot (gcc_x[1], p(gcc_x[1]), linewidth=0.2)
coef_gcc.append(round(z[0]*10**6,5))
print("licheepi_gcc_O1 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(gcc_x[2], gcc_y[2], 1)
p = np.poly1d (z)
plt.plot (gcc_x[2], p(gcc_x[2]), linewidth=0.2)
coef_gcc.append(round(z[0]*10**6,5))
print("licheepi_gcc_O2 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(gcc_x[3], gcc_y[3], 1)
p = np.poly1d (z)
plt.plot (gcc_x[3], p(gcc_x[3]), linewidth=0.2)
coef_gcc.append(round(z[0]*10**6,5))
print("licheepi_gcc_O3 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))
...
plt.ylabel("Time, s")
plt.savefig("result/licheepi_gcc.png")
...
llvm_y = [[],[],[],[]]
for i in range(4):
    with open("data/LicheePi4A/llvm-O" + str(i) + ".csv", "r") as f:
        j = 1
        for ch in f:
...
print("licheepi_llvm_O0 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(llvm_x[1], llvm_y[1], 1)
p = np.poly1d (z)
plt.plot (llvm_x[1], p(llvm_x[1]), linewidth=0.2)
coef_llvm.append(round(z[0]*10**6,5))
print("licheepi_llvm_O1 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(llvm_x[2], llvm_y[2], 1)
p = np.poly1d (z)
plt.plot (llvm_x[2], p(llvm_x[2]), linewidth=0.2)
coef_llvm.append(round(z[0]*10**6,5))
print("licheepi_llvm_O2 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))

z = np.polyfit(llvm_x[3], llvm_y[3], 1)
p = np.poly1d (z)
plt.plot (llvm_x[3], p(llvm_x[3]), linewidth=0.2)
coef_llvm.append(round(z[0]*10**6,5))
print("licheepi_llvm_O3 ",round(z[0]*10**6,5), " us/val", file=open("result/licheepi4a_result.txt", "a"))
...
plt.xlabel("Len of massive")
plt.ylabel("Time, s")
plt.savefig("result/licheepi_llvm.png")
